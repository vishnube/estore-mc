...
function setSelectedBatch() {
    ...
    var pdbch_exp = tr.find('.pdbch_exp').val();
    var pdbch_mrp = tr.find('.pdbch_mrp').val();
    ...
    $('#product_window #pw_pdbch_mrp').val(pdbch_mrp);
    $('#product_window #pw_pdbch_exp').val(pdbch_exp);
...
function setSelectedGodown(obj) {
    ...
    var billType = 'sls_bls';

    // Stock checking for sale bill
    ...
    $('#product_window #pw_gdn_name').val(obj.find('.gdn_name').val());
...
function loadProductBatches() {
    ...
    var cstr_id = $('#add_bill_modal #cstr_id').val();
    var fmly_id = $('#add_bill_modal #fmly_id').val();
    ...
    $('#product_window #tbl_pdbch_container').postForm(url, input, function (res, form, input) {
        ...
        $.each(res.product_batch_data, function (i, row) {
            ...
            str += "<input type='hidden' class='pdbch_exp' value='" + row.pdbch_exp + "'>"
            str += "<input type='hidden' class='pdbch_mrp' value='" + row.pdbch_mrp + "'>"
            ...
            str += "</tr>";
            str += "<tr class='gdn-row'><td colspan='6'>";
            $.each(row.gdn_stock, function (gdn_id, gdn_row) {
                $.each(gdn_row, function (ugp_id, ugp_row) {
                    if (ugp_row) {
                        ...
                        str += '<input type="hidden" class="gdn_name" value="' + ugp_row.gdn_name + '" >';
